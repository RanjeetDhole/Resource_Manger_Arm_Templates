...
def parse_arguments(args):
    ...
    parser.add_argument("--FolderName",dest="FolderName", help="FolderName name for pipeline", required=True)
    
    
    parser = argparse.ArgumentParser(description='Deploy SQL scripts to multiple databases on a single server.')
    parser.add_argument('--script_directory', type=str, default=os.path.join(os.getcwd(), args.FolderName),
                        help='Directory containing the SQL scripts')
...
def main():
    ...
    multi_script(server_config, args.script_directory)
